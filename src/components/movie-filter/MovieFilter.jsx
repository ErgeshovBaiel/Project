...
const MovieFilter = () => {
    ...
    return (
        <div className="pb-[80px]">
            <div className="app-container flex gap-[75px] items-center
              px-[75px]  rounded-[10px] bg-[#1A1A1A] text-white">
                {filterNames.map(item => {
                    let activeClass = item === activeBtn ? "border-b border-[red]" : ""
                    return <p onClick={() => {
                        handleOnClick  (item)
                    }} className={`${activeClass} cursor-pointer   py-[22px]`} key={item}>{item}</p>
                })}
            </div>
